 ...
 * a specified domain size. Variables can be registered multiple times, but
 * in each case the domain size must not change. Variables must always
 * be registered before they are referenced by any function.
 * 
 */
...
namespace maxsum
{
    ...
    * Paramters are iterators over a list of variable ids, or type maxsum::VarID
